                ...
                if (msg.startsWith("/block ") && msg.length > 7) {
                    ...
                    if (user === p) {
                        message({
                            s: "[CLIENT]",
                            r: 0,
                            m: `your STUPID and DUMB why would you try to block yourself LOLOLOLOL get HAXXED NOOOBBBB`
                        }, true);
                    } else if (devs.includes(p)) {
                        message({
                            s: "[CLIENT]",
                            r: 0,
                            m: `Seriously? Blocking a DEV? Ur goated, cuz the devs of this are probably annoying`
                        }, true);
                        blocked.push(p);
                        ...
                        message({
                            s: "[CLIENT]",
                            r: 0,
                            m: `User ${p.safe()} lol they got blocked already`
                        }, true);
                    } else {
                        blocked.push(p);
                        message({
                            s: "[CLIENT]",
                            r: 0,
                            m: `they can no longer decrease your brain cells ${p.safe()}`
                        }, true);
                        localStorage.setItem("blocked", blocked.join(" "));
                    }
                    ...
                    if (blocked.includes(p)) {
                        ...
                        message({
                            s: "[CLIENT]",
                            r: 0,
                            m: `Could not unblock user ${p.safe()}<br>because they are not in the blocked list, or something went wrong (maybe its a skill issue)`
                        }, true);
                    }
...
<li>/unflip &lt;message&gt; - [CLIENT] Appends deez nuts to the end of the message.</li>
<li>/msg &lt;message&gt; - [CLIENT] Sends a private message to all client users. which there are none cuz this client is localhost lol</li>
</ul>
With perms:<ul>
...
<li>/remove &lt;username&gt; - Removes ones' perms</li>
    ...
    settingsBtn.addEventListener("click", () => {
        ...
        else// in memory of the zazbugger...
            show(settingsDiv);
